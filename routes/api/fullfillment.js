...
router.post("/delete", async (req, res) => {
  ...
  const len = await Product.count({
    $or: [
      { fullfillment_amazon: req.query._id },
      { fullfillment_thirdparty: req.query._id },
      { fullfillment_us: req.query._id },
      { fullfillment_fba_fee: req.query._id },
    ],
  });
  if (len == 0) {
    ...
    return res
      .status(400)
      .send({
        message: "This fullfillment is already used in other products.",
      });
  }
});
