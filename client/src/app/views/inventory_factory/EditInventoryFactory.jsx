...
import { getInventoryFactoryById, updateFactoryReport } from "./InventoryFactoryService";
...
class EditInventoryFactory extends Component {
  state = {
    warehouseOptions: [],
  ...
  getInitialState = () => {
    getInventoryFactoryById(this.props.match.params.id).then((res) => {
      let submittedInfo = {
        userName: res.data.report.editted_user.name,
        modeifedAt: res.data.report.modified_at,
      }
      let reportList = [];
  ...
  handleSubmit = () => {
    ...
    updateFactoryReport(postData).then((res) => {
      this.getInitialState();
      this.setState({
        submitLoading: false,
        messageOpen: true,
...
export default EditInventoryFactory;
