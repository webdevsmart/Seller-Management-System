...
class EditProduct extends Component {
      ...
      this.setState(
        ...
        () => {
          getProductById(this.props.match.params.id).then(async (res) => {
            let UPCImage = await fetch(
              `/${res.data.upc_image.path}`
            ).then((r) => r.blob());
            let UPCImageFile = new File(
              [UPCImage],
              res.data.upc_image.file_name
            );
            UPCImage = {
            ...
            let FNSKUImage = await fetch(
              `/${res.data.fnsku_image.path}`
            ).then((r) => r.blob());
            let FNSKUImageFile = new File(
              [FNSKUImage],
              res.data.fnsku_image.file_name
            );
            FNSKUImage = {
            ...
            res.data.variation_qualities.map(async (item, index) => {
              getAllVariationValue(item.type).then((res) => {
            ...
            res.data.parts.map((part, index) => {
              selectedParts.push({
                ...part,
                label: part.name,
                value: part._id,
                partsQty: res.data.parts_qty[index],
              });
              selectedPartsIndex.push(part._id);
            });
            ...
            let curProduct = {
              ...
              ASIN: res.data.asin,
              selectedParentCategory: {
                ...res.data.parent_category,
                value: res.data.parent_category._id,
                label: res.data.parent_category.category,
              },
              ...
              selectedFFAmazon: {
                ...res.data.fullfillment_amazon,
                label: res.data.fullfillment_amazon.name,
                value: res.data.fullfillment_amazon._id,
              },
              selectedFFThirdParty: {
                ...res.data.fullfillment_thirdparty,
                label: res.data.fullfillment_amazon.name,
                value: res.data.fullfillment_amazon._id,
              },
              selectedFFUs: {
                ...res.data.fullfillment_us,
                label: res.data.fullfillment_amazon.name,
                value: res.data.fullfillment_amazon._id,
              },
              ...
              selectedFreight: {
                ...res.data.freight,
                cost_usd: res.data.freight.cost_usd,
                UM: res.data.freight.UM.short_name,
                value: res.data.freight._id,
                label: res.data.freight.name,
                ID: res.data.freight.ID,
              },
              freightQty: res.data.freight_qty,
              selectedStorage: {
                ...res.data.storage,
                rate: res.data.storage.rate,
                UM: res.data.storage.UM.short_name,
                value: res.data.storage._id,
                label: res.data.storage.name,
                ID: res.data.storage.ID,
              },
  ...
  handleSubmit = (event) => {
    ...
    this.state.productImages.map((image) => {
      if (image.isNew == true) formData.append("files", image.file);
    });
  ...
  handleSelectVType = (data, index) => {
    ...
    const check_type = variationQualities.filter(
      (variation) => variation.type.value === data.value
    );
              ...
              <Grid item lg={2} md={4} sm={6} xs={6} className="text-center">
                {selectedFreight && (
                  <TextField
                    ...
                    value={
                      "$ " +
                      parseFloat(selectedFreight.cost_usd * freightQty).toFixed(
                        2
                      )
                    }
                    readOnly
